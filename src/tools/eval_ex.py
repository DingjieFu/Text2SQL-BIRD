...
def run_sqls_parallel(sqls, db_places, num_cpus=16, meta_time_out=30.0):
    pool = mp.Pool(processes=num_cpus)
    for i, sql_pair in enumerate(sqls):
        predicted_sql, ground_truth = sql_pair
        # print("-"*50)
        # print(f"[Execute query id]: {i}")
        # print(f"[Predicted_SQL]: {predicted_sql}")
        # print(f"[Ground_Truth_SQL]: {ground_truth}")
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--save_metric', action="store_true", help='whether to save metric file')
    args = args_parser.parse_args()
    exec_result = []

    print(f"[Eval the predicted SQL in: {args.predicted_sql_path}]\n")

    # pred SQLs
    pred_queries, db_paths = package_sqls(args.predicted_sql_path, args.db_root_path, mode='pred', data_mode=args.data_mode)
    # ground truth SQLs:
    ...
    """
    if args.save_metric:
        metric_file = args.predicted_sql_path + f"predict_{args.data_mode}_metric.json"
        levels = ['simple', 'moderate', 'challenging', 'total']
        result = {
            "counts": dict(zip(levels, count_lists)),
            "accuracy": dict(zip(levels, score_lists))
        }
